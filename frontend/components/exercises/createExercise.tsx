...
const CreateExercise: React.FC<Props> = ({ exercises }) => {
  ...
  const handleSubmit = async (event: { preventDefault: () => void }) => {
    ...
    try {
      ...
      setStatusMessage([
        { message: "Exercise succesfully added.", type: "success" },
      ]);
      setTimeout(() => {
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mt-2">
          ...
          <div className="block mb-2 text-sm font-medium bg-[#393b3d] rounded-lg">
            ...
            >
              <option value="" className="bg-[#393b3d] text-white">
                Select a type
              </option>
              {filteredTypes.map((type, index) => (
                <option
                  key={index}
                  value={type}
                  className="bg-[#393b3d] text-white"
                >
                  {type}
                </option>
              ))}
