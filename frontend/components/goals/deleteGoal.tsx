...
const DeleteGoal: React.FC<props> = ({ goalId }) => {
    ...
    const handleSubmit = async (event: { preventDefault: () => void }) => {
        ...
        try {
            ...
            setStatusMessage([{ message: "Goal succesfully deleted.", type: "success" }])
            setTimeout(() => {
                router.push("/goals");
            }, 1000);
        } catch (error) {
    ...
    return (
        <div className="max-w-sm m-auto">
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className="flex gap-x-1 mt-2 justify-center">
                    <Link href={`/goals`} className="text-white bg-gray-700 hover:bg-gray-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
                        Cancel
                    </Link>
                    <button
