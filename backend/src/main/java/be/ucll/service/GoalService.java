...
public class GoalService {
    ...
    public Goal createGoal(Goal goal) {
        User user = userRepository.findById(goal.getUserId())
                .orElseThrow(() -> new RuntimeException("User with id " + goal.getUserId() + " does not exist."));
        if (goal.getStartDate().isAfter(goal.getEndDate())) {
    ...
    public Goal editGoal(Goal changed_goal, String id) {
        Goal old_goal = goalRepository.findById(id)
        ...
        // Change goal in user
        User user = userRepository.findById(changed_goal.getUserId())
                .orElseThrow(() -> new RuntimeException("User with id " + changed_goal.getUserId() + " not found."));
    ...
    public void deleteGoalById(String id) {
        // Get Goal by id
        Goal deleted_goal = goalRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Goal with id " + id + " not found."));
        // Remove goal from user
        User user = userRepository.findById(deleted_goal.getUserId())
                .orElseThrow(() -> new RuntimeException("User with id " + deleted_goal.getUserId() + " not found."));
